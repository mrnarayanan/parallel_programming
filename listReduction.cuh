...
__global__ void total(float *input, float *output, int len) 
{
  ...
  if (start + tx < len)
    ...
    partialSum[tx] = 0.0;
  if (start + bd + tx < len)
    partialSum[bd + tx] = input[start + bd + tx];
  else
    partialSum[bd + tx] = 0.0;
  
  for (int stride = bd; stride >= 1; stride >>= 1) // right shift by 1 place is same as divide by 2
  {
    __syncthreads();
    if (tx < stride)
  ...
  if (tx == 0)
    output[blockIdx.x] = partialSum[tx];
}
...
int main(int argc, char **argv) {
  ...
  dim3 DimGrid(ceil(numInputElements/(float) BLOCK_SIZE/2),1,1);
