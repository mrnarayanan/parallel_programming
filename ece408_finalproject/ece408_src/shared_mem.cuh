...
#define k4d(i3, i2, i1, i0) k[(i3) * (C * K * K) + (i2) * (K * K) + (i1) * (K) + i0]
    ...
    int loc_x_i = threadIdx.x + blockIdx.x * TILE_WIDTH;
    int loc_y_i = threadIdx.y + blockIdx.y * TILE_WIDTH;
    int loc_z_i = threadIdx.z + blockIdx.z * 1;

    int loc_x_o = threadIdx.x + blockIdx.x * TILE_WIDTH;
    int loc_y_o = threadIdx.y + blockIdx.y * TILE_WIDTH;
    int loc_z_o = threadIdx.z + blockIdx.z * 1;
    ...
    if ((ty < TILE_WIDTH) && (tx < TILE_WIDTH)){ // if dimensions are within input bounds and M
        for (int m = 0; m < M; m++){    // Loop through output features
            ...
            if ((loc_x_o < W_out) && (loc_y_o < H_out) && (loc_z_o < B)){  // if thread is within output bounds
                y4d(loc_z_o, m, loc_y_o, loc_x_o) = conv_val;  // set convolution result
            }
        }
    }
