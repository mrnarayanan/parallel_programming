...
__global__ void matrixMultiplyShared(float *A, float *B, float *C,
  ...
  for (int ph = 0; ph < ceil(numAColumns/(float)TILE_WIDTH); ph++)
  {
    ...
    if ( (Row < numCRows) && ((ph * TILE_WIDTH + tx) < numAColumns) )
      subTileA[ty][tx] = A[Row * numAColumns + ph * TILE_WIDTH + tx];
    else
      subTileA[ty][tx] = 0.0;
    if ( ((ph * TILE_WIDTH + ty) < numBRows) && (Col < numCColumns) )
      subTileB[ty][tx] = B[(ph * TILE_WIDTH + ty) * numBColumns + Col];
    else
      subTileB[ty][tx] = 0.0;
...
int main(int argc, char **argv) {
  ...
  int gridY = ceil((1.0*numCRows)/TILE_WIDTH);
  int gridX = ceil((1.0*numCColumns)/TILE_WIDTH);
